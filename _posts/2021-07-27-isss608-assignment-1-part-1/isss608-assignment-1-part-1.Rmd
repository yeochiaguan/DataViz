...
```{r}
#Changed facet to comments as Netlify is unable to display facets clearly
...
We start off by plotting the route taken by each employee for each day by id. The below example is a plot on 06 Jan 2014.
...
```{r}
#This code chunk is for displaying Route based on Employment Title, but due to issues showing on netlify, snipshots of the interactive maps are pasted instead.

#plotting the path by Employment Title to identify outliers. The assessment was done, but it was unable to b published on netlify, hence the bottom part of the code is taken out.
